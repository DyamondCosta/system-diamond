...
async function salvarVendaPneu(cliente, forma_pagamento, observacao) {
    ...
    if (quantidade > Number(pneu.quantidade)) {
        alert('Estoque insuficiente! Disponível: ' + pneu.quantidade);
        return;
    }
...
function renderizarVendas(lista) {
    ...
    lista.forEach(venda => {
        ...
        <div class="card">
            ...
            <p>
                <strong style="color:#10b981;">R$ ${Number(venda.valor_total || 0).toFixed(2)}</strong>
                ${venda.lucro ? `<span style="color:#8b5cf6;margin-left:10px;">Lucro: R$ ${Number(venda.lucro).toFixed(2)}</span>` : ''}
            </p>
            <p style="color:#6b7280;">${venda.forma_pagamento}</p>
            <button onclick="excluirVenda(${venda.id})" style="background:#ef4444;">🗑️ Excluir</button>
        </div>`;
    });
}
